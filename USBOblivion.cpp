    ...
    , m_psapi    ( _T( "psapi.dll" ) )
{
	...
	FuncLoad( m_advapi32, RegDeleteTreeW );
...
BOOL CUSBOblivionApp::InitInstance()
{
    ...
    if ( szArglist )
    {
        for( int i = 1; i < nArgs; ++i )
        {
            ...
            // Them x? ly cac tham s? m?i
            else if ( CmpStrI( szArglist[ i ], _T("-clearbrowser") ) || CmpStrI( szArglist[ i ], _T("/clearbrowser") ) )
            {
                dlg.m_bClearBrowser = TRUE;
            }
            else if ( CmpStrI( szArglist[ i ], _T("-clearwifi") ) || CmpStrI( szArglist[ i ], _T("/clearwifi") ) )
            {
                dlg.m_bClearWifi = TRUE;
            }
            else if ( CmpStrI( szArglist[ i ], _T("-clearapps") ) || CmpStrI( szArglist[ i ], _T("/clearapps") ) )
            {
                dlg.m_bClearApps = TRUE;
            }
        }
    }
    LocalFree( szArglist );
    ...
    if ( bHelp )
    {
        // C?p nh?t thong tin tr? giup d? bao g?m cac tham s? m?i
        CString helpText = LoadString( IDS_ABOUT );
        helpText += _T("\n-clearbrowser - Clear browser history\n-clearwifi - Clear Wi-Fi history\n-clearapps - Clear recent apps history");
        AfxMessageBox( helpText, MB_OK | MB_ICONINFORMATION );
    }
    else
    {
        m_pMainWnd = &dlg;
